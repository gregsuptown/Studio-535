/**
 * IMPROVED VERSION of server/db.ts
 *
 * Key improvements:
 * 1. Added database-level filtering for better performance
 * 2. Added pagination support
 * 3. Better error handling
 * 4. Added query optimizations
 */
...
/**
 * IMPROVED: Added pagination support
 */
export async function getAllProjects(limit: number = 100, offset: number = 0) {
  const db = await getDb();
  if (!db) return [];
  return db
    .select()
...
/**
 * NEW: Get projects by email with pagination (database-level filtering)
 * This is much more efficient than fetching all projects and filtering in code
 */
...
/**
 * NEW: Get project count by email (for pagination)
 */
export async function getProjectCountByEmail(email: string): Promise<number> {
  const db = await getDb();
  if (!db) return 0;
...
/**
 * IMPROVED: Better typing for status
 */
export async function updateProjectStatus(
  id: number,
  status: "intake" | "design" | "approval" | "production" | "fulfillment" | "completed" | "cancelled"
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
/**
 * IMPROVED: Added pagination support for messages
 */
